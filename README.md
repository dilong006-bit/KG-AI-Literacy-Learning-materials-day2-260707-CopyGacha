...
**🔗 라이브 데모:** https://kg-ai-literacy-learning-materials-d-sand.vercel.app/
...
스킬 기반 워크플로로 단계적으로 고도화했으며, 각 단계는 **로컬 검증(빌드·Playwright) 후 커밋**했습니다.

1. **접근성 개선** — 전역 키보드 포커스 링, 오류 텍스트 AA 대비, 모바일 터치 타깃 ≥44px, `prefers-reduced-motion` 대응
2. **표준 정합 리팩토링** — 프로젝트 규칙 기준 죽은 코드·중복 제거(동작 보존), 엔트리 id 충돌 버그(`performance.now()` 클램핑) 수정
3. **v4.0 이미지 페어링 (F20~F22)** — Unsplash 무상태 서버리스 프록시 + 카피-이미지 페어링(카드별 다른 이미지) + 프리미엄 비주얼(스크림·blur-up·어트리뷰션)
4. **Vercel 배포 + 트러블슈팅** — 환경변수 키 오염(공백/개행) 방어, 카드 이미지 배너 정렬(flex 음수마진 비대칭 → 너비 명시) 해결
